...
const visit=suite=>{for(const child of suite.suites??[])visit(child);for(const spec of suite.specs??[])for(const test of spec.tests??[]){
 ...
 const projectArea=spec.file?.includes('/public/')?'Public':spec.file?.includes('/admin/')?'Admin':spec.file?.includes('/api/')?'API':requestedFeature==='preview'?'Preview':'Other';
 tests.push({feature,suite:suiteName,testName:spec.title,projectArea,browser:test.projectName??browser,viewport:null,status:statusNumber(final.status),durationMs:final.duration??0,retryCount:Math.max(0,results.length-1),isFlaky:test.status==='flaky',errorType:final.error?.name??null,errorSummary:(final.error?.message??final.error?.stack??null)?.slice(0,2000)??null,sourceFile:spec.file??null,startedAtUtc:started?.toISOString()??null,completedAtUtc:completed?.toISOString()??null,correlationKey:correlation});
...
function featureFrom(file,fallback){const normalized=(file??'').replaceAll('\\','/');const match=normalized.match(/playwright\/(?:admin|public|api|preview|visual|recording)\/([^/]+)/);return match?.[1]??fallback;}
