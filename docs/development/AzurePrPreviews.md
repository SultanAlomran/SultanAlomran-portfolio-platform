...
3. Create a protected GitHub environment named `azure-preview`. Add the printed `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_LOCATION`, `AZURE_PREVIEW_RESOURCE_GROUP`, and `AZURE_RESOURCE_SUFFIX` as environment variables. Add `AZURE_PREVIEW_ADMIN_EMAIL` as an environment variable for the disposable Preview administrator. Add `AZURE_SQL_ADMIN_PASSWORD` and a separate strong `AZURE_PREVIEW_ADMIN_PASSWORD` as environment secrets.
...
Add the `azure-preview` label to an open PR targeting `dev`. Open/reopen, label, and subsequent synchronize events validate builds, build images, deploy/migrate/seed, run a focused remote Playwright smoke suite, import its normalized metadata, and update one marker comment with HTTPS Web, Admin, API, health, Scalar, and OpenAPI URLs. The GitHub environment also displays the Web URL. Different PR numbers have separate concurrency groups, apps, databases, and telemetry; a newer run supersedes an older run for the same PR.
...
Preview SQL contains the application schema, deterministic safe demonstration data, and normalized test analytics metadata generated by Playwright for that preview. It does not contain copied LocalDB data, Production data, or historical local Quality Dashboard data. Screenshots, videos, traces, HTML reports, ZIP files, and other evidence remain GitHub Actions artifacts; SQL stores only `TestRun`, `TestCaseResult`, and `TestArtifact` metadata and provider links. Import runs through a Preview-only CI command using the existing application importer and its provider/run idempotency key; the unauthenticated HTTP import route remains Development-only.

`Preview` is an explicit ASP.NET Core environment. Scalar and `/openapi/v1.json` are enabled there for review and remain disabled in Production. Admin and `/api/admin/**` require the isolated Preview Administrator created from the protected GitHub environment; arbitrary Google accounts remain denied and Google is disabled unless separately configured. Preview uses Secure, HttpOnly, `SameSite=None` cookies because Admin and API have distinct HTTPS hosts, plus explicit credentialed CORS and antiforgery. Frontends also send `X-Robots-Tag: noindex, nofollow, noarchive`, but indexing controls are not authentication. Never place production secrets, personal data, private messages, or confidential files in a preview.
