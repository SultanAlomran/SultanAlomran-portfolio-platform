...
internal sealed class MediaService(PortfolioDbContext db, IMediaStorage storage) : IMediaService
{
    ...
    public async Task<MediaPage> ListAsync(MediaQuery query, CancellationToken token)
    {
        ...
        var total = await source.CountAsync(token); var page = Math.Max(query.Page, 1); var size = Math.Clamp(query.PageSize, 1, 100);
    ...
    { var file = await db.MediaFiles.AsNoTracking().SingleOrDefaultAsync(x => x.Id == id, token); if (file is null) return null; return Map(file, await ReferencedIds(token)); }
    public async Task<MediaFileDto> UploadAsync(MediaUpload upload, CancellationToken token)
    {
        ...
        await using var memory = new MemoryStream(); await upload.Content.CopyToAsync(memory, token);
        var bytes = memory.ToArray(); if (!MatchesSignature(allowed, bytes)) throw new InvalidMediaException("The file contents do not match its extension.");
        var key = $"{DateTime.UtcNow:yyyy/MM}/{Guid.NewGuid():N}{extension}";
        memory.Position = 0; await storage.StoreAsync(key, memory, token);
        try { var entity = MediaFile.Create(Path.GetFileName(key), Path.GetFileName(upload.OriginalFileName), storage.GetUrl(key), allowed, upload.Length, "local"); db.MediaFiles.Add(entity); await db.SaveChangesAsync(token); return Map(entity, []); }
        catch { await storage.DeleteAsync(key, token); throw; }
    }
    public async Task DeleteAsync(Guid id, CancellationToken token)
    {
        ...
        db.MediaFiles.Remove(file); await db.SaveChangesAsync(token); await storage.DeleteAsync(file.FilePath, token);
    }
    ...
    {
        var ids = await db.ProjectImages.Select(x => x.MediaFileId).Concat(db.Projects.Where(x => x.ThumbnailMediaFileId != null).Select(x => x.ThumbnailMediaFileId!.Value)).Concat(db.Infographics.Where(x => x.CoverMediaFileId != null).Select(x => x.CoverMediaFileId!.Value)).Concat(db.Infographics.Where(x => x.InfographicMediaFileId != null).Select(x => x.InfographicMediaFileId!.Value)).Concat(db.Infographics.Where(x => x.PdfMediaFileId != null).Select(x => x.PdfMediaFileId!.Value)).Concat(db.MediaCollectionItems.Select(x => x.MediaFileId)).Distinct().ToListAsync(token); return ids.ToHashSet();
    }
    private static bool MatchesSignature(string type, byte[] b) => type switch { "image/png" => b.Length > 8 && b.AsSpan(0,8).SequenceEqual(new byte[]{137,80,78,71,13,10,26,10}), "image/jpeg" => b.Length > 3 && b[0]==255 && b[1]==216 && b[2]==255, "image/webp" => b.Length > 12 && b.AsSpan(0,4).SequenceEqual("RIFF"u8) && b.AsSpan(8,4).SequenceEqual("WEBP"u8), "application/pdf" => b.Length > 5 && b.AsSpan(0,5).SequenceEqual("%PDF-"u8), _ => false };
}
