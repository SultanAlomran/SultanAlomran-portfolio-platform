...
@Injectable({providedIn:'root'}) export class MediaApiService {
 ...
 private withPublicUrl(item:MediaFile):MediaFile{return{...item,url:item.url.startsWith('/')?`${new URL(environment.apiUrl,globalThis.location.origin).origin}${item.url}`:item.url}}
}
